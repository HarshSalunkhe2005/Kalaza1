...
- **`isMinifyEnabled` still `false`** on the release build type — no code shrinking/obfuscation. `proguard-rules.pro` now has the standard kotlinx.serialization keep rules pre-added (every `@Serializable` *Row DTO needs these or release-only `SerializationException`s are likely) so flipping it later is a smaller step, but it hasn't actually been turned on or tested in a real release build (no Android SDK in this sandbox to build one) — do that as its own deliberate step, not blindly.
...
- **Supabase account-level personal access token pasted in chat** (section 25) — same handling: revoke from Supabase dashboard → Account → Access Tokens once active development winds down. It's account-wide (every project on the account), not scoped to just this one.
...
- **Resolved for Super Admin**: off-network login now works via the password-gated bypass in the Wi-Fi dialog (section 14). Staff/Supervisor still have no off-network path — explicitly the next thing being discussed (section 26), not started yet.
- **Stale merged feature branches on GitHub** (`fix-tag-encode-default`, `scan-tab-refinements`, `schema-audit-fixes`, `claude/repo-access-check-sl1ml8`) still can't be deleted — both `git push --delete` and a direct GitHub API `DELETE` (tried again with the Supabase work, still 403) are blocked by this environment's proxy (`"Write access to this GitHub API path is not permitted through this proxy"`). Cosmetic only; would need deleting from an environment without that restriction.

---

## What is Remaining (Future Scope)

Backend integration, real authentication, push notification delivery, real-time sync, offline support, and per-day medication history (all previously listed here as open) are now **done** — see Technology Stack and section 1 above, "Push Notification System"/"Medication Deadline Reminders & Escalation" below, and sections 27 (offline) and 30 (per-day history) above. What's actually left:

### 1. Test Account Coverage (Low Priority)
...
### 2. Security Hardening & Wi-Fi-Scoped Auth (Next Up)
